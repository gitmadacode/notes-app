...
if(errors.length > 0){
    ...
    newNote.user = req.user.id; //esto para que enlace las notas de ESE usuario
...
router.get('/notes', isAuthenticated, async (req,res) => {  //async por que buscar tambien es un proceso asincrono
    const notes = await Note.find(user: req.user.id).sort({date: 'desc'}); //desde la coleccion note quiero buscar todos los datos,sort es para que se ordenen segun yo quiera,si voy a buscar todoas las notras muestrame solo las que coincidan con mi id
...
router.delete('/notes/delete/:id',isAuthenticated, async (req,res)=>{                                //con esto remuevo el id de la bd
