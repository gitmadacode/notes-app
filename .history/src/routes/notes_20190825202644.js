...
router.get('/notes', isAuthenticated, async (req,res) => {  //async por que buscar tambien es un proceso asincrono
    const notes = await Note.find().sort({date: 'desc'}); //desde la coleccion note quiero buscar todos los datos,sort es para que se ordenen segun yo quiera,si voy a buscar todoas las notras muestrame solo las que coincidan con mi id
